//Colin Spratt
//Prim from textbook authors modified to find lowest Latency Spanning Tree
//4-3-18

import java.util.*;
public class PrimMin{
  private Edge[] edgeTo;
  private double[] distTo;
  private boolean[] marked;
  private IndexMinPQ<Double> pq;

  public PrimMin(Graph G){
    edgeTo = new Edge[G.V()];
  ...
  private void scan(Graph G, int v){
    ...
    for(int i = 0; i < adj.size(); i++){
      Edge e = adj.get(i);
      double weight = e.time(); //Negate bandwidth to find max
